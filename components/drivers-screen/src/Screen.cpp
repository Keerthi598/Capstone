...
void LCD_I2C::init(uint8_t addr, uint8_t dataPin, uint8_t clockPin, uint8_t cols, uint8_t rows)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(100));                                  // 4.1 mS delay (min)
    LCD_writeNibble(LCD_FUNCTION_RESET, LCD_COMMAND);                   // second part of reset sequence
    vTaskDelay(pdMS_TO_TICKS(100));                                                  // 100 uS delay (min)
    ...
    vTaskDelay(pdMS_TO_TICKS(100));                                                   // 40 uS delay (min)
    ...
    vTaskDelay(pdMS_TO_TICKS(100));                                   // Clearing memory takes a bit longer
...
void LCD_I2C::print_pred(int pred, float confidence)
{
    clear();
    setCursor(0, 0);
    print("The scanner is ");

    setCursor(0, 1);
    int temp_b = confidence * 100;
    std::string temp_a = std::to_string(temp_b) + "% confident: ";
    print(temp_a);
...
void LCD_I2C::home(void)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(100));                                   // This command takes a while to complete
}
